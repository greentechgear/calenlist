...
-- Create a function to check if a user can access a calendar
CREATE OR REPLACE FUNCTION can_view_calendar(calendar_id uuid)
RETURNS boolean AS $$
BEGIN
  RETURN EXISTS (
...
-- Create a function to check if a user can subscribe to a calendar
CREATE OR REPLACE FUNCTION can_subscribe_to_calendar(calendar_id uuid)
RETURNS boolean AS $$
BEGIN
  RETURN EXISTS (
    SELECT 1 FROM calendars c
...
CREATE POLICY "subscriptions_insert"
  ...
  WITH CHECK (
    ...
    can_subscribe_to_calendar(calendar_id)
  );
