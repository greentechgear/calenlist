...
-- Create improved profiles read policy
CREATE POLICY "profiles_read_policy"
  ...
  USING (
    ...
    EXISTS (
      ...
      AND (
        c.is_public = true OR
        -- Also allow reading profile if user has access to their private calendar
        (auth.role() = 'authenticated' AND c.user_id = profiles.id)
      )
    )
  );
